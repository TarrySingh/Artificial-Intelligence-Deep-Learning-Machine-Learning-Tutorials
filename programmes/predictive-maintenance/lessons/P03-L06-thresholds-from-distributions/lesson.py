...
# the same cost evaluated at the real slots in a workshop calendar, two schedulers — the one a
# planner writes on a whiteboard, and the one that is actually cheapest — for a workshop that
# cannot take two machines on the same Thursday, and the bridge that takes module 5's
# predictive draws exactly as they arrive.
#
...
# operating point in currency), `P03-L05-remaining-useful-life` (the degradation posterior
# and `rul_samples()`, whose draws section 13 prices). Sections 2 to 12 render their
# forecasts in this notebook, for the reason section 1 gives.
#
# By the end you will be able to:
#
# 1. Implement the expected cost of intervening at hour *t* given a predictive RUL
#    distribution, locate the cost-optimal intervention time, and measure how far it is from
#    the mean RUL on your own numbers.
# 2. Measure how the optimal intervention time moves when the *width* of the distribution
#    changes and its centre does not — and why, from the hazard rate you compute yourself.
# 3. Price a plan against a maintenance calendar with real slots in it, rather than against a
#    continuous clock.
# 4. Implement a capacity-constrained schedule, and measure what the workshop's one-machine-
#    per-slot rule costs against the unconstrained optimum — and what a first-come-first-served
#    planner costs on top of that.
# 5. Implement the bridge from module 5's predictive draws to this module's forecast, and
#    measure what reading those draws through a mean and a standard deviation costs.
#
...
_EXERCISES: dict[str, tuple[str, ...]] = {
    ...
    "exercise 10": ("forecast_from_samples",),
}
...
# shape directly from its mean and standard deviation, because holding the mean fixed while
# moving the width is the experiment of section 7, and no sample of draws holds a mean fixed.
# Module 5 itself hands over *draws*; section 13 is where they arrive, through exercise 10,
# and everything between here and there consumes `RULForecast` and nothing else.
#
...
# model to one unit's trajectory, hand its draws to exercise 10's `forecast_from_samples`,
# and everything below this cell takes the result unchanged.
...
def make_forecast(unit: int, mean_rul: float, sd_rul: float, made_at: int = NOW,
    ...
    crossing time whose right tail is long. Given to you; it stands in for module 5's output
    until section 13 takes module 5's own draws.
    """
...
def _check_forecast_summary() -> None:
    ...
    try:
        forecast_summary(RULForecast(0, 0, np.array([0, 1]), np.array([0.5, 0.500005])))
    except ValueError:
        pass
    else:
        raise AssertionError(
            "a pmf summing to 1.000005 is outside the docstring's 1e-6 and must raise "
            "ValueError. np.isclose(total, 1.0) lets it through, because its default tolerance "
            "is about 1e-5: compare abs(total - 1.0) with 1e-6 itself")
    try:
        forecast_summary(RULForecast(0, 0, np.array([0, 1]), np.array([0.5, 0.4999995])))
    except ValueError:
        raise AssertionError(
            "a pmf summing to 0.9999995 is within the docstring's 1e-6 of 1 and must be "
            "summarised, not rejected: a tolerance tighter than 1e-6 refuses a forecast that "
            "was merely rounded") from None
...
def survival_and_hazard(forecast: RULForecast) -> tuple:
    ...
    Example:
        ...
        >>> [round(float(v), 4) for v in s], [round(float(v), 4) for v in h]
...
def _check_realised_cost() -> None:
    ...
    try:
        overdue = realised_cost(3, 0, cheap)
    except ValueError:
        raise AssertionError(
            "realised_cost(3, 0) raised ValueError. A true_rul of 0 is a machine that fails "
            "during hour 0, not a negative remaining life: only a negative one is refused"
        ) from None
    assert overdue == 100.0, (
        f"a machine that fails during hour 0 has failed before an intervention at hour 3; "
        f"expected 100.0, got {overdue!r}"
    )
    for bad in ((-1, 5), (5, -1)):
        try:
...
def slot_costs(forecast: RULForecast, slots, prices: Prices) -> np.ndarray:
    ...
    Example:
        ...
        >>> p = Prices(planned=10.0, unplanned=100.0, life_per_hour=1.0)
        >>> slot_costs(f, [8, 10, 11], p).round(2).tolist()
        [inf, 20.5, 55.6]
    """
    # YOUR CODE HERE
    raise NotImplementedError
...
# %% [markdown]
# ## 13. Exercise 10 — `forecast_from_samples()`: module 5's draws, as they arrive
#
# Every forecast so far came out of `make_forecast`, which renders a shape from two numbers.
# Module 5 does not hand over two numbers. Its `rul_samples()` pushes the posterior over the
# degradation line through the crossing time and returns *draws*: a sample of remaining lives
# for one unit, clipped into `[0, RUL_CAP]`, carrying whatever tail that ratio produces.
#
# The next cell carries a faithful copy of `rul_samples()`, with module 5's constants under
# module 5's names, so this notebook still runs on its own. It draws for four posteriors of
# the kind module 5 fits at its first checkpoint, and prints how far each sample is from
# symmetric.

# %%
# Module 5's vocabulary, copied with its names so the two notebooks read the same. The
# failure level is a definition from module 4's labelling policy, on module 5's log scale.
HEALTHY = 1.0
FAILURE_LEVEL = 9.0
...
def rul_samples(mean: np.ndarray, cov: np.ndarray, t_now: float, y_fail: float,
    ...
    Module 5's exercise 3, copied so this notebook runs alone; given to you. Draw `n_samples`
    pairs `(a, b)` from `N(mean, cov)` using `np.random.default_rng(seed)` — same seed, same
    draws. For each draw the crossing time is `(y_fail - a) / b` and the remaining life is
    ...
    draws = rng.multivariate_normal(mean, cov, size=int(n_samples))
    intercept, slope = draws[:, 0], draws[:, 1]
    with np.errstate(divide="ignore", invalid="ignore"):
        crossing = (float(y_fail) - intercept) / slope
    rul = np.where(slope <= 0.0, RUL_CAP, crossing - float(t_now))
    return np.clip(rul, 0.0, RUL_CAP)


# Four posteriors over [intercept, slope] of module 5's log-degradation line, as its
# fit_posterior returns them at its first checkpoint (hour 96) for its units 100, 102, 103 and
# 106. With a fixed noise level the posterior covariance depends only on the reading times, so
# the four share one. Rounded from a run of module 5's reference; inputs here, not measurements.
M5_CHECKPOINT = 96
M5_COV = np.array([[1.009e-2, -1.361e-4], [-1.361e-4, 2.864e-6]])
M5_POSTERIORS = {100: np.array([-2.362, 0.00968]), 102: np.array([-3.178, 0.00851]),
                 103: np.array([-2.802, 0.00793]), 106: np.array([-3.042, 0.00895])}
M5_DRAWS = {unit: rul_samples(mean, M5_COV, M5_CHECKPOINT, Y_FAIL, seed=SEED + unit)
            for unit, mean in M5_POSTERIORS.items()}


def skewness(x) -> float:
    """Third standardised moment: 0 for any symmetric shape, positive for a long right tail."""
    x = np.asarray(x, dtype=float)
    return float(((x - x.mean()) ** 3).mean() / x.std() ** 3)


print(f"module 5's draws at its hour-{M5_CHECKPOINT} checkpoint, {N_SAMPLES} per unit")
print(f"{'unit':>4} {'mean':>7} {'sd':>6} {'skewness':>9} {'past the grid':>14}")
for _unit, _draws in M5_DRAWS.items():
    print(f"{_unit:>4} {_draws.mean():>7.1f} {_draws.std():>6.1f} {skewness(_draws):>9.2f} "
          f"{(_draws > GRID_HOURS).mean():>14.1%}")

# %% [markdown]
# Turning draws into this module's forecast is one function, and three conventions in it
# decide every number printed after it. They are section 1's, applied to a sample:
#
# - A draw `r` belongs to the hour during which the unit fails: bin `k` holds the draws with
#   `k < r <= k + 1`. That is the convention `make_forecast`'s pmf already follows.
# - A draw of exactly 0 is module 5's clip of an overdue unit. It has failed, so it goes in
#   bin 0, where even an intervention at hour 0 loses it.
# - A draw past the end of the grid is folded into the last bin, as `make_forecast` folds its
#   tail. It is never dropped, and nothing is renormalised to hide it.
#
# Every draw weighs the same, so the pmf is the share of the sample in each bin.
#
# <details><summary>💡 Hint 1 — what to think about</summary>
#
# Take a draw of 6.0 and a draw of 6.5 and decide, from the first bullet, which bin each
# belongs to; the rounding function you reach for first may disagree about one of them. Then
# push a draw of 0.0 through the same arithmetic: what index comes out, and where does numpy
# put a value written at that index? Finally, what should a draw of nan or of -3 produce,
# given that module 5 clips everything it returns into a range?
# </details>
# <details><summary>💡 Hint 2 — the approach, in words</summary>
#
# Validate first: an empty sample, a grid below one hour, or any draw that is negative or not
# finite is a `ValueError`. Turn each draw into a bin index by rounding up and stepping back
# one, then hold the index inside the grid at both ends, so the overdue draws land in the
# first bin and the long ones in the last. Count the draws in each bin, divide by the number
# of draws, and return a `RULForecast` carrying the unit, the hour it was made and an integer
# hour grid.
# </details>

# %%
def forecast_from_samples(samples, unit: int = 0, made_at: int = NOW,
                          grid_hours: int = GRID_HOURS) -> RULForecast:
    """This module's forecast, built from module 5's predictive draws rather than from moments.

    Bin `k` of the grid holds the share of the draws with `k < r <= k + 1`: the hour during
    which the unit fails, section 1's convention. A draw of exactly 0 (module 5's clip of an
    overdue unit) belongs in bin 0, and a draw beyond the grid is folded into the last bin, so
    no draw is dropped and the pmf sums to 1 without renormalising.

    Raise `ValueError` on an empty sample, a `grid_hours` below 1, or any draw that is
    negative or not finite: module 5 clips its draws into `[0, RUL_CAP]`, so any of those is a
    bug upstream, and repairing it here would hide it.

    Returns a `RULForecast(unit, made_at, hours, pmf)` with `hours = np.arange(grid_hours)`.

    Example:
        >>> f = forecast_from_samples([0.0, 1.0, 1.5, 7.0], unit=3, grid_hours=4)
        >>> f.unit, f.hours.tolist(), f.pmf.tolist()
        (3, [0, 1, 2, 3], [0.5, 0.25, 0.0, 0.25])
    """
    # YOUR CODE HERE
    raise NotImplementedError


def _check_forecast_from_samples() -> None:
    toy = forecast_from_samples([0.0, 1.0, 1.5, 7.0], unit=3, made_at=5, grid_hours=4)
    assert (int(toy.unit), int(toy.made_at)) == (3, 5), (
        f"the forecast must carry the unit and the hour it was made, got unit {toy.unit!r} and "
        f"made_at {toy.made_at!r}. slot_costs reads made_at, so a dropped one moves every slot"
    )
    assert (np.asarray(toy.hours).tolist() == [0, 1, 2, 3]
            and np.issubdtype(np.asarray(toy.hours).dtype, np.integer)), (
        f"hours must be the INTEGER grid 0 .. grid_hours - 1, got {np.asarray(toy.hours)!r}. "
        "make_forecast builds its hours as ints, and a forecast from draws must be the same "
        "record as a forecast from moments; a float grid compares equal and is not"
    )
    edges = forecast_from_samples([0.25, 1.0, 1.75, 3.0, 3.5], grid_hours=5)
    assert np.allclose(edges.pmf, [0.4, 0.2, 0.2, 0.2, 0.0], atol=1e-12), (
        f"draws 0.25, 1.0, 1.75, 3.0, 3.5 belong in bins 0, 0, 1, 2, 3, giving "
        f"[0.4, 0.2, 0.2, 0.2, 0.0]; got {np.round(np.asarray(edges.pmf), 4).tolist()}. A "
        "draw of exactly 1.0 fails during hour 0, because bin k holds k < r <= k + 1: floor() "
        "puts it one bin late, ceil() without stepping back puts every draw one bin late, and "
        "rounding to the nearest hour moves half of them"
    )
    overdue = forecast_from_samples([0.0, 0.0, 2.5], grid_hours=4)
    assert np.allclose(overdue.pmf, [2 / 3, 0.0, 1 / 3, 0.0], atol=1e-12), (
        f"two overdue draws of 0.0 belong in bin 0; got "
        f"{np.round(np.asarray(overdue.pmf), 4).tolist()}. If they landed in the LAST bin, "
        "the index came out as -1 and numpy read it from the end: the machines that have "
        "already failed became the longest-lived ones on the grid"
    )
    tail = forecast_from_samples([1.5, 50.0, RUL_CAP], grid_hours=10)
    assert np.asarray(tail.pmf).size == 10 and abs(float(np.sum(tail.pmf)) - 1.0) < 1e-12, (
        f"a 10-hour grid gives 10 bins summing to 1; got {np.asarray(tail.pmf).size} bins "
        f"summing to {float(np.sum(tail.pmf))!r}. Draws past the grid are folded, not dropped"
    )
    assert abs(float(tail.pmf[-1]) - 2 / 3) < 1e-12, (
        f"draws of 50 and {RUL_CAP:.0f} h are past a 10-hour grid and fold into its last bin, "
        f"which should hold 2/3; got {float(tail.pmf[-1]):.4f}. Dropping them and "
        "renormalising what is left makes the unit look closer to failure than module 5 said"
    )
    for bad, grid, why in (([], 4, "an empty sample"), ([-0.5, 3.0], 4, "a negative draw"),
                           ([math.nan, 3.0], 4, "a nan draw"),
                           ([math.inf, 3.0], 4, "an infinite draw"),
                           ([1.5, 2.5], 0, "a grid of 0 hours")):
        try:
            forecast_from_samples(bad, grid_hours=grid)
        except ValueError:
            continue
        raise AssertionError(f"{why} must raise ValueError: module 5 never returns one, so it "
                             "is a bug upstream and not something to repair here")
    try:
        single = forecast_from_samples([0.5, 2.0, RUL_CAP], grid_hours=1)
    except ValueError:
        raise AssertionError(
            "grid_hours=1 raised ValueError. The docstring refuses a grid BELOW one hour; a "
            "one-hour grid is the smallest legal one, and every draw folds into its only bin"
        ) from None
    assert np.allclose(single.pmf, [1.0], atol=1e-12), (
        f"on a one-hour grid every draw folds into bin 0, so the pmf is [1.0]; got "
        f"{np.asarray(single.pmf).tolist()}"
    )
    draws = M5_DRAWS[103]
    real = forecast_from_samples(draws, unit=103)
    share = np.array([np.mean(draws <= k + 1) for k in range(GRID_HOURS - 1)])
    assert np.allclose(np.cumsum(real.pmf)[:-1], share, atol=1e-9), (
        "on module 5's unit 103 the running total of your pmf at hour k must equal the share "
        "of draws with r <= k + 1, and it does not. Check the binning rule against the first "
        "bullet above"
    )
    print(f"exercise 10 looks right — module 5's unit 103: {draws.size} draws, "
          f"{float(real.pmf[-1]):.1%} of them in the last bin, pmf sums to "
          f"{float(np.sum(real.pmf)):.6f}")


# %%
_try("exercise 10", _check_forecast_from_samples)

# %%
def normal_forecast(unit: int, mean_rul: float, sd_rul: float, made_at: int = NOW,
                    grid_hours: int = GRID_HOURS) -> RULForecast:
    """A normal with the given mean and sd, on the grid: what 'mean ± sd' on a dashboard implies.

    Given to you: `make_forecast`'s symmetric sibling. The mass a normal puts at or below 0 h
    is a unit that has already failed, so it goes in bin 0; the tail beyond the grid is folded
    into the last bin, as before.
    """
    if mean_rul <= 0 or sd_rul <= 0:
        raise ValueError("mean_rul and sd_rul must both be positive")
    cdf = np.array([_standard_normal_cdf((k - mean_rul) / sd_rul)
                    for k in range(grid_hours + 1)])
    pmf = np.diff(cdf)
    pmf[0] += cdf[0]                   # mass at or below 0 h: failed already, so hour 0
    pmf[-1] += 1.0 - cdf[-1]           # fold the tail beyond the grid into the last bin
    pmf = np.maximum(pmf, 0.0)
    pmf /= pmf.sum()
    return RULForecast(unit=int(unit), made_at=int(made_at),
...
# Each unit is planned three ways: from module 5's draws through YOUR forecast_from_samples,
# and from two shapes that keep only the draws' mean and sd. Every plan is then priced under
# the draws, because the draws are the belief module 5 actually hands over. The plans come
# from the sweep, which integrates the pmf; a hazard read bin by bin off a sample is noisy.
def _draws_against_moments() -> None:
    print("extra cost: each plan priced under the draws, less the draws' own plan.\n"
          "P(fails first): the chance of failing at or before the hour the NORMAL books,\n"
          "as the draws see it and as the normal sees it.\n")
    print(f"{'':>4} {'booked hour':^24} {'extra cost':^19} {'P(fails first)':^20}")
    print(f"{'unit':>4} {'draws':>7} {'normal':>7} {'lognorm':>8} {'normal':>9} {'lognorm':>9} "
          f"{'draws':>8} {'normal':>10}")
    base = extra_normal = extra_lognorm = 0.0
    earlier, shift, risk_ratio, over_quote = 0, 0, [], []
    for unit, draws in M5_DRAWS.items():
        empirical = forecast_from_samples(draws, unit=unit)
        mean, sd = float(draws.mean()), float(draws.std())
        shapes = {"draws": empirical, "normal": normal_forecast(unit, mean, sd),
                  "lognorm": make_forecast(unit, mean, sd)}
        plans = {name: best_intervention_time(f, CANDIDATES, SCENARIO)
                 for name, f in shapes.items()}
        priced = {name: expected_cost_at(empirical, plan.hours_ahead, SCENARIO)
                  for name, plan in plans.items()}
        extra = {name: priced[name] - priced["draws"] for name in ("normal", "lognorm")}
        assert min(extra.values()) >= -1e-6, (
            f"unit {unit}: a plan made from a two-moment shape came out CHEAPER under the draws "
            f"than the plan made from the draws themselves ({extra}). The draws' plan is the "
            "minimum of that very curve, so forecast_from_samples, expected_cost_at and "
            "best_intervention_time disagree somewhere"
        )
        booked = plans["normal"].hours_ahead
        fails_draws = 1.0 - float(survival_and_hazard(empirical)[0][booked])
        fails_normal = 1.0 - float(survival_and_hazard(shapes["normal"])[0][booked])
        base += priced["draws"]
        extra_normal += extra["normal"]
        extra_lognorm += extra["lognorm"]
        earlier += int(max(plans["normal"].hours_ahead, plans["lognorm"].hours_ahead)
                       < plans["draws"].hours_ahead)
        shift = max(shift, max(abs(plans[name].hours_ahead - plans["draws"].hours_ahead)
                               for name in ("normal", "lognorm")))
        risk_ratio.append(fails_normal / fails_draws)
        over_quote.append(plans["normal"].cost / priced["normal"] - 1.0)
        print(f"{unit:>4} {plans['draws'].hours_ahead:>7} {plans['normal'].hours_ahead:>7} "
              f"{plans['lognorm'].hours_ahead:>8} {extra['normal']:>9,.0f} "
              f"{extra['lognorm']:>9,.0f} {fails_draws:>8.1%} {fails_normal:>10.1%}")
    n = len(M5_DRAWS)
    print(f"\nacross the {n} units, booking from the normal costs {extra_normal:,.0f} more than "
          f"booking from the draws ({extra_normal / base:.1%}),\nand booking from this module's "
          f"own log-normal costs {extra_lognorm:,.0f} more ({extra_lognorm / base:.1%}). Both "
          f"two-moment shapes\nbook earlier than the draws on {earlier} of {n} units, by at "
          f"most {shift} hours.")
    print(f"\nat the hour the normal books, it puts the chance of failing first at "
          f"{min(risk_ratio):.1f}x to {max(risk_ratio):.1f}x what module 5's\ndraws say, and "
          f"quotes its own plan {min(over_quote):.1%} to {max(over_quote):.1%} dearer than the "
          f"draws price it.")
    if extra_normal / base < min(over_quote) and min(risk_ratio) > 1.0:
        print("The two moments cost less in the plan than they misstate in the risk and the "
              "price that\ntravel with it — and those are the numbers a schedule, a budget "
              "and a sign-off read.")


_try("module 5's draws, priced", _draws_against_moments,
     needs=("exercise 2", "exercise 3", "exercise 4", "exercise 10"))
...
#   one is free to fix.
# - **Collapsing module 5's draws to a mean and a standard deviation.** Section 13 priced it
#   twice: once in the plan, and once in the risk and the price reported with the plan.
# - **Binning a draw with `floor`, or letting an overdue draw index bin -1.** The first files
#   every draw that lands on a whole hour one bin late; the second files the machines that
#   have already failed as the longest-lived ones on the grid.
...
#
# 6. Module 5's draws for a unit have a long right tail, and you hand the planner a normal
#    with the same mean and sd instead. Priced against the draws, what goes most wrong is:
#    - (a) nothing: a mean and an sd are all a distribution carries into an expected cost
#    - (b) the booked hour, by weeks, because the normal has no long right tail
#    - (c) the risk and the price quoted with the plan, because both are read off the near
#          tail, and two moments fitted to a skewed sample do not pin the near tail down
#
# Answers, with the reasoning, come with this lesson's worked solution when you enrol on Synapsa.
...
# by a calendar and by a workshop that takes one machine at a time. Exercise 10 plugged it into
# module 5 as module 5 actually delivers: draws, priced as they are rather than as two moments.
#
...
if __name__ == "__main__":
    with contextlib.redirect_stdout(io.StringIO()):
        for _name, _check in (("exercise 1", _check_forecast_summary),
                              ...
                              ("exercise 10", _check_forecast_from_samples)):
            _try(_name, _check)
    _progress_board()
