...
**10 of 11 modules built.** A module is built when its lesson directory exists and has passed all 14 gates in `QUALITY.md` and an independent review; everything else is specified here and has no directory. Seconds and peak memory are measured by `tools/execute.py`; rubric size is read from each lesson's `tests/test_lesson.py`. Regenerate with `python tools/status.py`.
