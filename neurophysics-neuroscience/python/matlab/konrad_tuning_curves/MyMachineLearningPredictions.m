...
%advanced exercise: is exponential better than linear-threshold?
...
%And now lets do the trivial linear things as a benchmark
...
b = TreeBagger(numberTrees,X,Y,'Method','R','OOBPred','On',...
'MinLeafSize',minLeafSize);
% how to know the crossvalidation error: oobError(b)
% keep in mind that only the relative error matters. So It is oobError(b)-mseTotalConst
% what are the right settings for numbertrees
plot(oobError(b)-mseTotalConst,col(i)); %lets only plot how much better we are doing
